...
## Spatial Parameters
a=0; b=1; # Total interval for spatial points
T0=0.0; Tf=2.0; # Total interval for temporal points

## Geometry Parameters -- used in both FG CB or FG Lam
eps=(b-a)/2; tau=(Tf-T0)/2; #These are the spatial-temporal periods for CB
m1=.5; n1=.5; #These are the spatial-temporal volume fractions of the periods

# Functionally Graded(FG) smoothing properties in space and time -
# Larger values give more smoothing
smt_x=.1; smt_t=.1 #Spatial-Temporal Smoothing
...
###############################################################################
## Plotting Parameters
M=400; N=400; # Number of sample points along x and t for fine plot
###############################################################################
...
Mat1=0; Mat2=255; #Material Range from Material 1 to Material 2
t_SampPeri=1/1000. # Time Sample Period in Milliseconds
N_SampTime=int((Tf-T0)/t_SampPeri) # Number of sample times
###############################################################################

x_SpatElem=np.linspace(a,b,N_SpatElem) # Evenly spaced array of spatial location of elements from a to b 
t_SampTime=np.linspace(T0,Tf,N_SampTime); # Evenly spaced array of time values from T0 to Tf 

X_Samp,T_Samp=np.meshgrid(x_SpatElem,t_SampTime); # Creates a meshgrid array of elements to capture all of the possible locations in space-time
xx_Samp=X_Samp.flatten(order='F'); tt_Samp=T_Samp.flatten(order='F');# Flattens the previously created meshgrid (in Fortran Order) to allow for faster calculation
...
zz_g=np.array(list(map(lambda x,t: g(x,t,Mat1,Mat2,m1,n1,eps,tau,smt_x,smt_t),xx_Samp,tt_Samp))) #FG CBD on Shive Devices
zz_fine=np.array(list(map(lambda x,t: g(x,t,Mat1,Mat2,m1,n1,eps,tau,smt_x,smt_t),xx,tt))) #FG CBD on Fine Mesh
...
indx_Const=np.where(np.abs(np.gradient(Z_g,axis=0)) <= GradTol); # Approximately constant cells
indx_Chang=np.where(np.abs(np.gradient(Z_g,axis=0)) >= GradTol); # Changing material property cells

K_g[indx_Chang]=Z_g[indx_Chang]; K_g[indx_Const]=-100
